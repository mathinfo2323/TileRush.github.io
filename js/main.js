...
class base_scene{
    constructor(){
        this.scene_start_time=performance.now();
        this.scene_current_time=performance.now();
        this.delta_time=0.0;
        this.past_time=0.0;
        this.frame_count=0;
        this.counter1=0;
    ...
    basic_refresh(){
        this.scene_current_time=performance.now();
        this.delta_time=this.scene_current_time-this.past_time;
        this.past_time=this.scene_current_time;
        this.frame_count+=1;
    }
}
...
class regular_mode_scene extends base_scene{//コピペして使う
    ...
    draw(){
        ...
        if(this.now_phase==game_phase.gameover){
            ...
            ctx.fillText(`スコア:${this.score}`,100,270);
            ctx.fillText(`速度:${(this.score/(this.end_time-this.start_time)*1000.0).toFixed(2)}打/秒`,100,340);
